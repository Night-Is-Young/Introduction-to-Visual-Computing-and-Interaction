...
\paragraph{Jacobian IK}
我们先来介绍一些有关Jacobian矩阵的知识.考虑非线性变换$(x,y)\mapsto(u,v)$,其微分关系为
\[\di u=\dfrac{\p u}{\p x}\di x+\dfrac{\p u}{\p y}\di y,\quad\di v=\dfrac{\p v}{\p x}\di x+\dfrac{\p v}{\p y}\di y\]
写成矩阵的形式则有
\[\begin{bmatrix}
    \di u\\\di v
\end{bmatrix}=\begin{bmatrix}
    \dfrac{\p u}{\p x}&\dfrac{\p u}{\p y}\\
    \dfrac{\p v}{\p x}&\dfrac{\p v}{\p y}\\
\end{bmatrix}\begin{bmatrix}
    \di x\\\di y
\end{bmatrix}\]
记$\di\vec{a}=\begin{bmatrix}
    \di x\\\di y
\end{bmatrix},\di\vec{b}=\begin{bmatrix}
    \di u\\\di v
\end{bmatrix},\mat{J}=\begin{bmatrix}
    \dfrac{\p u}{\p x}&\dfrac{\p u}{\p y}\\
    \dfrac{\p v}{\p x}&\dfrac{\p v}{\p y}\\
\end{bmatrix}$,则有
\[\di\vec{b}=\mat{J}\di\vec{a}\]
这就描述了两个坐标系之间的非线性变换的性质,矩阵$\mat{J}$被称作这一变换的Jacobian矩阵.\\
\indent 现在我们考虑刚体铰链模型,末端位置$\vec{p}=\begin{bmatrix}
    x&y&z
\end{bmatrix}^\t$仅与各个关节的旋转角度$\theta_i$有关.记$\bs\theta=\begin{bmatrix}
    \theta_1&\cdots&\theta_n
\end{bmatrix}^\t$,类似地就有
\[\di\vec{p}=\mat{J}\di\bs\theta\]
我们的目标是为了确定末端位置如何从一个位置$\vec{p}_e$移动到另一个位置$\vec{p}_t$,这要求系统末端的位移为$\Delta\vec{p}=\vec{p}_e-\vec{p}_t$.当位移$\Delta\vec{p}$取的足够小时,我们就根据上述微分式近似地得到
\[\Delta\vec{p}=\mat{J}\Delta\bs\theta\]
然后对式子两边左乘Jacobian矩阵的逆即可得到
\[\Delta\bs\theta=\mat{J}^{-1}\Delta\vec{p}\]
根据位置变换$\Delta\vec{p}$即可求得各关节的角度变化$\Delta\bs\theta$.
\subsection{动作捕捉}
...
\indent 在进行运动方向对齐前,我们先定义角色的\tbf{朝向坐标系(Facing Frame)}.
\begin{definition}[朝向坐标系]
    朝向坐标系是固定在角色上的一个局部坐标系,可以用参数$(\mat{R},\vec{t})$表示;旋转矩阵$\mat{R}$表示局部坐标系在世界坐标系下的旋转,平移向量$\vec{t}$表示局部坐标系的原点在世界坐标系下的位置.
\end{definition}
一般而言, $\vec{t}$取角色的根关节在世界坐标系下的位置, $\mat{R}$则是一个绕$y$轴的旋转,满足朝向坐标系的$z$轴指向角色面朝的方向.\\
\indent 现在,记transition frame时刻$A,B$序列中的朝向坐标系分别为$\mat{R}_{A0},\vec{t}_{A0}$和$\mat{R}_{B0},\vec{t}_{B0}$,我们需要对$B$序列的朝向坐标系进行变换$\mathcal{T}$使得其与动作$A$的朝向坐标系在初始状态下先对齐,也即让动作$A$和动作$B$在连接开始时具有相同的朝向;然后考虑transition frame后的某一帧$i$, 在这一帧时动作序列$A$和$B$中的朝向坐标系分别为$\mat{R}_A(i),\vec{t}_A(i)$和$\mat{R}_B(i),\vec{t}_B(i)$.我们对$\mat{R}_B(i),\vec{t}_B(i)$做相同的变换$\mathcal{T}$,就得到了对齐后的动作$B'$,然后对$A$与$B'$进行动作连接即可.\\
\indent 考虑在帧$i$时刻任取一个$\mat{R}_A(i),\vec{t}_A(i)$坐标系下的三维坐标$\vec{x}$,设其转换为$\mat{R}_{A0},\vec{t}_{A0}$坐标系下的坐标为$\vec{x}_0$;取$\mat{R}_B(i),\vec{t}_B(i)$坐标系下的同样的坐标$\vec{x}$,设其转换为$\mat{R}_{B0},\vec{t}_{B0}$坐标系下的坐标为$\vec{x}_1$,那么应当有如下关系:
\[\vec{x}_0=\vec{x}_1\]
