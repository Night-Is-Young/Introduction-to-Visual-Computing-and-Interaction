...
\end{aligned}\]
最后归一化即可得到新的法向量:
\[\vec{n}'=\text{normalize}\left(\vec{n}-\dfrac{\p h}{\p u}\vec{b}\times\vec{n}-\dfrac{\p h}{\p v}\vec{n}\times\vec{t}\right)\]
\indent 在实际实现中,我们通常使用屏幕坐标及其偏导数代替前述推导中的UV坐标.考虑屏幕上位于$(x,y)$处对应物体的点在空间中的位置$\vec{p}(x,y)$.对其求偏导可得
\[\vec{p}_x=\dfrac{\p\vec{p}}{\p x},\quad\vec{p}_y=\dfrac{\p\vec{p}}{\p y}\]
然后令
\[\vec{r}_1=\vec{p}_y\times\vec{n},\quad \vec{r}_2=\vec{n}\times\vec{p}_x\]
于是同理可得
\[\vec{n}'=\text{normalize}\left(\vec{n}-\dfrac{\p h}{\p x}\vec{r}_1-\dfrac{\p h}{\p y}\vec{r}_2\right)\]
