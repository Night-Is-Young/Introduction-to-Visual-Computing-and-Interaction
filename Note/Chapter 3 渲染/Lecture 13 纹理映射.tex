...
对于模型上任意一点,将其位置$\vec{p}$写做UV坐标系下的参数方程$\vec{p}=\vec{p}(u,v)$.为了求出这一点的法向量$\vec{n}$,我们考虑固定$u$得到的曲线$\vec{p}_v=\vec{p}(u_0,v)$和固定$v$得到的曲线$\vec{p}_u=\vec{p}(u,v_0)$.对这两条曲线分别求导,可得它们在此处的切向方向为
