...
为了描述表面对光的反射特性,我们引入\tbf{双向反射分布函数(Bidirectional Reflectance Distribution Function, BRDF)}. BRDF描述了入射光线和出射光线之间的关系,具体而言是对于物体表面上的一点$\vec{p}$,我们希望知道沿$\bs\omega_i$方向入射光的辐照度$E_i(\vec{p})$在沿$\bs\omega_o$方向上造成的出射光的辐射率$L_o(\vec{p},\bs\omega_o)$.
