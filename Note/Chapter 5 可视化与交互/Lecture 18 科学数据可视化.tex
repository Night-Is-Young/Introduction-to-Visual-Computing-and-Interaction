...
\end{definition}
设流场在时刻$t_0$时的速度场为$\vec{v}(\vec{x},t_0)$.引入一个积分参数$s$,流线$\vec{x}(s)$可以定义为
\[\dfrac{\di\vec{x}(s)}{\di s}=\vec{v}(\vec{x}(s),t_0),\quad\vec{x}(0)=\vec{x}_0\]
流线反映了一个稳态流场(或非稳态流场的某个瞬时)的结构,它是与时间$t$无关的量.
\begin{definition}[路径线]
    \tbf{路径线(Pathline)}是指在流场中,某一特定流体粒子随时间运动所形成的轨迹.路径线描述了单个流体粒子的运动过程.
\end{definition}
对于一个在时刻$t_0$处于位置$\vec{x}_0$的粒子,将其随时间变化的轨迹记为$\vec{x}(t)$,则有
\[\dfrac{\di\vec{x}(t)}{\di t}=\vec{v}(\vec{x}(t),t),\quad\vec{x}(t_0)=\vec{x}_0\]
这里的$\vec{x}(t)$即为该粒子的路径线.
\begin{definition}[迹线]
    \tbf{迹线(Traceline)}是指同一空间位置持续释放的流体粒子在流场中随时间运动所形成的轨迹.迹线反映了流体粒子的历史运动路径.
\end{definition}
对于在时间$\tau$被释放的粒子,用$\vec{x}(t;\tau)$表示其在$t\geq\tau$时所处的位置,则有
\[\dfrac{\di\vec{x}(t;\tau)}{\di t}=\vec{v}(\vec{x}(t;\tau),t),\quad\vec{x}(\tau;\tau)=\vec{x}_0\]
\paragraph{点噪声法}
\tbf{点噪声法(Dot Noise)}的基本思想是在采样区域随机设置噪声点,并将它们的形状,方向,拉伸程度与局部向量信息相对应,从而在整体上形成密集且连续的流动纹理效果.\\
\indent 可以用简单的线性组合描述图像:
\[I(\vec{x})=I_0+\sum_{i}\alpha_i\cdot h(\vec{x}-\vec{i})\]
其中$I_0$为本底颜色(即背景), $\alpha_i$为每个扩散核的系数, $h$为扩散核函数.
\paragraph{线性积分卷积}
\tbf{线性积分卷积(Line Integral Convolution, LIC)}的基本思想是沿着流场的流线对噪声进行采样和卷积.
