...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    if (!u_UseBlinn) {
        return lightIntensity * (diffuseColor * max(dot(n, l), 0.0) + specularColor * pow(max(dot(v, reflect(-l, n)), 0.0), shininess));
    } else{
        return lightIntensity * (diffuseColor * max(dot(n, l), 0.0) + specularColor * pow(max(dot(h, n), 0.0), shininess));
    }
}
